...

{
  _ship = _x;
    if (_ship getVariable ["tf47service", false]) then {
      ...
      _shipServ1 = _ship addAction [("<t color=""#fa8f3e"">" + ("Repair Vehicle") + "</t>"), "tf47\vehicleService\vehicleRepairSubroutine.sqf", [_ship]];
    	_shipServ2 = _ship addAction [("<t color=""#fa8f3e"">" + ("Refuel Vehicle") + "</t>"), "tf47\vehicleService\vehicleRefuelSubroutine.sqf", [_ship]];
    	_shipServ3 = _ship addAction [("<t color=""#fa8f3e"">" + ("Rearm Vehicle") + "</t>"), "tf47\vehicleService\vehicleAmmoSubroutine.sqf", [_ship]];

      if (_addLoadouts == 1) then {
        _shipServ4 = 0;

        _shipServ4 addAction [("<t color=""#fadf3e"">" + ("Loadout: Boat Infantry") + "</t>"), "tf47\vehicleService\vehicleLoadoutSubroutine.sqf", [_ship, "BoatInf"]];
      };


      if (_respawnactive) then {
        _shipServ99 = _ship addAction [("<t color=""#fa4f3e"">" + ("Respawn Vehicle") + "</t>"), "tf47\vehicleService\vehicleRespawnSubroutine.sqf", [_ship]];
      };
